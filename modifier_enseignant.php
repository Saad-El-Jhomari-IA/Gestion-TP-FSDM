...
<div class="div1">
    <div class="row">
        ...
        <ul class="navbar-nav">
        <li class="nav-item">
          <a class="nav-link "  href="Ajouter_Enseignant_Admin.php" style="color:blue;"> <B> Ajouter Enseignant </B></a>
        </li>
        <li class="nav-item">
        <a class="nav-link" href="supprimer_enseignant.php" style="color:BLUE;"><b>supprimer Enseignant</b></a>
        </li>
...
<h1 align="center" >Modifier Un Enseignant</h1>
<div align="center" class="card" style="width: 50%;margin-left:300px;margin-top:40px">
    <h2>Entrer le code d'enseignant que vous voulez modifier</h2>
<form method = "post">
...
if(isset($_POST["Code"])){
    $_SESSION["CODEN"]=$_POST["Code"];
    try {
        $cn1 = new PDO("mysql:host=localhost;dbname=tp;port=3306;charset=utf8", 'root', '');
    } catch(PDOException $erreur) {
        die("Erreur de connexion : " . $erreur->getMessage());
    }
    $req = "SELECT `cneens`, `nomen`, `passworden`, `web_dynamique`, `reseaux`, `projet_tutore`, `projet_tutore2`, `base_de_donnee`, `ro`,count(*) as 'pass' FROM `enseignant` WHERE cneens='".$_POST["Code"]."' and nomen='".$_POST["Nom"]."';";
    $rep = $cn1->query($req);
    while($ligne=$rep->fetch()){
        if($ligne["pass"]==1){
            ?>
        <!-- <form action="verification2_Enseignant.php" method="post">
            <label for="Nom1">Entrez un nouveau nom et prénom :</label>
            <input type="text" name="Nom1" id="Nom1" required/>
            <label for="Pass1">Affecter un mot de passe :</label>
            <input type="password" name="Pass1" id="Pass1" required/>
            <label for="web_dynamique">Web dynamique :</label>
            <input type="checkbox" name="web_dynamique" value="1" id="web_dynamique"/>
            <label for="ro">Recherche opérationnel :</label>
            <input type="checkbox" name="ro" value="1" id="ro"/>
            <label for="base_de_donnee">Base de données :</label>
            <input type="checkbox" name="base_de_donnee" value="1" id="base_de_donnee"/>
            <label for="projet_tutore">Projet tutoré :</label>
            <input type="checkbox" name="projet_tutore" value="1" id="projet_tutore"/>
            <label for="projet_tutore2">Projet tutoré 2 :</label>
            <input type="checkbox" name="projet_tutore2" value="1" id="projet_tutore2"/>
            <label for="reseaux">Réseaux :</label>
            <input type="checkbox" name="reseaux" value="1" id="reseaux"/>
            <input type="submit" value="Envoyer"/>
            <input type="reset" value="Annuler"/>
        </form> -->
        <div class="card"  align="center" style="width: 50%; margin-left:300px;margin-top:40px;">
        <h3 align="center">Modification d'enseignant <?=$_POST["Nom"]?></h3>
   
   <form style="margin-top: 20px;" action="" method="post"  class="my-form">
 ...
 <div class="form-group">
   <label>Modifier les modules pour Enseignant</label><br>
   <div class="form-check">
     <input class="ABBBBBBBBBB" class="ABBBBBBBBBB" style="margin-left: 250px;" class="form-check-input" type="checkbox" name="web_dynamique" id="web_dynamique" value="1">
...
if(isset($_POST["Code"])&&isset($_POST["Pass1"])&&isset($_POST["Nom1 "])){
    ...
    $rep1=$cn1->exec($req1);
    if($rep1){ ?>
        <div  style="width: 20%;margin-left:800px" class="alert alert-success"><b align="center"><?php echo "Enseignant ".$_POST["Nom"]." est modifier";?></b></div><?php 
    }else{
        echo "Update failed";
    }
}
